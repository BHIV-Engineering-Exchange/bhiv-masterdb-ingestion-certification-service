...
| Bucket (Siddhesh Narkar) | No integration code exists. No live endpoint, credentials, or confirmed evidence-publishing contract received. | — |
...
| InsightFlow/InsightBridge/InsightCore (Vijay Dhawan) | `GET /metrics` exists, Prometheus text format — chosen as a reasonable default (also named in the original brief's Learning Kit), NOT a confirmed contract. Vijay Dhawan needs to confirm this is the expected shape/format before treating this as done. | `main.py`, `tests/test_runtime_identity_and_metrics.py` |
